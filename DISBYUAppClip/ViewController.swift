...
//  DISBYUAppClip
//
//  Created by Trevor Schmidt on 4/16/21.
//
...
class ViewController: UIViewController {
    ...
    private func pullInfoOfBathroom() {
        ...
        URLSession.shared.dataTask(with: url!, completionHandler: { (data, response, error) in
            ...
            var reviewsList: [fireReview] = [fireReview]()
            ...
            for key in bathroomData.keys {
                if key == "name" {
                    ...
                    for reviewKey in reviewsDict.keys {
                        ...
                        let newReview = fireReview(userID: userID, username: username, title: title, comments: comments, date: date, rating: rating, upvotes: upvotes, userLike: userLike)
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ReviewCelll", for: indexPath) as! ReviewCelll
        ...
        if reviews.count == 0 {
            ...
            cell.commentsLabel.text = "Be the first to write one by hitting the link above."
            return cell
        }
